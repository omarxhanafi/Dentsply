...
export default class Flow_recordVisualPicker extends LightningElement {
    ...
    handleSubmit(event) {
        ...
        if (this.objectApiName === 'Contact') {
           ...
           if (this._recordId.substring(0, 3) === '001') fields.AccountId = this._recordId;
           fields.InsertEmailMarketingConsentCollected__c = 'No';
        }
    ...
    handleError(event){
        console.log(JSON.stringify(event.detail),'errorr');
        this.template.querySelector('[data-id="formerror"]').setError(JSON.stringify(event.detail.detail)); 
    }
    handleCloseModal() {
        this.newContact = false;
    }
